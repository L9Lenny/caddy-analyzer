...
- **Guard mode refactored**: Extracted `Guard` type with `Evaluate()`, `Tick()`, `Run()` methods and `Blocker` interface for testable iptables mocking. `runGuard` is now thin orchestration. Audit logging via `OnAudit` callback. State persistence to `--state-file` (default `/var/lib/caddy-analyzer/blocked.json`) — blocked IPs and expiry timestamps survive crashes/restarts. Expired entries are cleaned up on load.
