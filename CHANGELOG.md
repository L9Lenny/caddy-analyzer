...
- **Guard mode refactored**: Extracted `Guard` type with `Evaluate()`, `Tick()`, `Run()` methods and `Blocker` interface for testable iptables mocking. `runGuard` is now thin orchestration. Audit logging via `OnAudit` callback.
